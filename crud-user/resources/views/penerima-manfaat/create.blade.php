...
<div class="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Tambah Penerima Manfaat</h2>
    ...
    <form action="{{ route('penerima-manfaat.store') }}" method="POST" class="space-y-6">
        ...
        <div class="space-y-2">
            ...
            <input type="text" 
                   ...
                   required
                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            <p class="text-xs text-gray-500">Masukkan 16 digit NIK</p>
        </div>
        ...
        <div class="space-y-2">
            ...
            <select id="panti_id" 
                ...
                @foreach($pantiSosials as $pantiSosial)
                    <option value="{{ $pantiSosial->id }}" {{ old('panti_id') == $pantiSosial->id ? 'selected' : '' }}>
                        {{ $pantiSosial->nama }}
                    </option>
                @endforeach
            </select>
