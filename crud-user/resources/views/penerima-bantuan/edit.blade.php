...
<div class="p-6">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Edit Penerima Bantuan</h2>
    </div>
        ...
        <form action="{{ route('penerima-bantuan.update', $penerimaBantuan->id) }}" method="POST">
                ...
                <select name="penerima_manfaat_id" 
                    ...
                    @foreach($penerimaManfaats as $penerimaManfaat)
                        <option value="{{ $penerimaManfaat->id }}" 
                                {{ old('penerima_manfaat_id', $penerimaBantuan->penerima_manfaat_id) == $penerimaManfaat->id ? 'selected' : '' }}>
                ...
                <select name="bantuan_id" 
                    ...
                    @foreach($bantuans as $bantuan)
                        <option value="{{ $bantuan->id }}" 
                                {{ old('bantuan_id', $penerimaBantuan->bantuan_id) == $bantuan->id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <input type="date" 
                       name="tanggal_terima" 
                       id="tanggal_terima" 
                       value="{{ old('tanggal_terima', $penerimaBantuan->tanggal_terima) }}"
                       class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                       required>
